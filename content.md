...
потому что, без использования внешних фреймворков можно легко и быстро реализовать свою библиотеку, 
...
Для того что бы выделить из всего множества слов некоторое его подмножество используют **[формальные грамматики](https://ru.wikipedia.org/wiki/Формальная_грамматика)**.
...
В первые о парсер комбинаторах начали говорить в 1980 годах, а на текущий момент для разных языков программирования существуют удобные библиотеки для работы с ними.
...
Теперь все готово, что бы написать наш первый парсер `TakeIf`. Он будет проверять - соответствует ли первый символ строки предикату.
```kotlin
...
а вторая - это рекурсивный вызов функции самой себя. Со второй проблемой легко справиться, добавив "ленивости" нашим парсерам:
```kotlin
...
```
Функция `seq` будет так же как и `alt` принимать на вход набор парсеров и последним аргументов функцию преобразования, например, в случае для трех парсеров ее сигнатура будет иметь вид:
```kotlin
...
Приведу пример для трех аргументов, а более подробно с кодом можно познакомиться в [репозитории](https://github.com/zewalldev/habr_combinators/blob/main/src/parsers/helpers.kt):
```kotlin
...
- `manyOf(p: Parser<A>): Parser<List<A>>` - пытается применить парсер `p` к входящей строке столько раз, сколько получиться, если не получилось ни разу, то вернет пустой список.
...
Многие компиляторы и IDE умеют показывать предупреждения если в функии форматного вывода, например `printf("%s = %f", "2 + 2", 5)`,
...
так как парсеры, разбирающие аргументы зависят от строки со спецификацией формата вывода. Т
о есть нам нужно научить нашу библиотеку комбинаторов выбирать на этапе разбора.
