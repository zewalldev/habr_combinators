...
Напишем синтаксические анализаторы для арифметических выражений и функций вида `"%s = %d".format("2 + 2", 5)`, в которых типы аргументов определяются строкой формата.
...
которые умеют подсвечивать ошибки в случае аргументов неправильного типа в коде вида:
```kotlin
"%s = %d".format("2 + 2", 5)
```
...
Дополнительно введем функцию, которая возвращает парсер, проверяющий соответствие первого символа строки заданному, она нам пригодится в будущем:
```kotlin
...
Приведу пример для трех аргументов, а более подробно с кодом можно познакомиться в [репозитории]():
```kotlin
